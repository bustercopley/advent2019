...
template <int N>
std::array<int64_t, N> get_params(
  const program_t &program, int64_t opcode, int64_t &pc, int64_t &base, int out, bool verbose) {
...
int64_t run_until_output(
  program_t &program, int64_t &pc, int64_t &base, std::vector<int64_t> &inputs, bool verbose) {
  while (true) {
    ...
    case 9: { // EQUALS
      ...
      if (verbose) {
        std::cout << "base = base + " << p[0] << " = " << (base + p[0]) << std::endl;
      }
      base += p[0];
      break;
    }
...
int CALLBACK _tWinMain(HINSTANCE, HINSTANCE, LPTSTR, int) {
  try {
    bool verbose = false;
    auto test_programs = read("9-test.data");
    auto programs = read("9.data");
