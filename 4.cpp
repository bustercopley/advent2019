...
bool ok(std::string s, bool part_two) {
  bool monotone = true;
  bool has_double = false;
  int stretch = 1;
  char c = s[0];
  for (int n = 1; std::size_t(n) != std::size(s); ++n) {
    char d = s[n];
    if (c == d) {
      ++stretch;
      if (!part_two && stretch == 2) {
        has_double = true;
      }
    } else {
      if (stretch == 2) {
  ...
  return has_double && monotone;
}

int count(bool part_two) {
  int n = 0;
  for (int i = 136818; i != 685979 + 1; ++i) {
    std::ostringstream stream;
    stream << i;
    if (ok(stream.str(), part_two)) {
      ++n;
    }
  }
  return n;
...
int CALLBACK _tWinMain(HINSTANCE, HINSTANCE, LPTSTR, int) {
  std::cout << count(false) << ", " << count(true) << std::endl;
  return 0;
}
